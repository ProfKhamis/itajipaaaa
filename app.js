...
// DOM Bindings - Strategy 7 (Bulk High Tick / Low Tick)
const btnBuyHTLT = document.getElementById('btn-buy-htlt');
const btnToggleAutoHTLT = document.getElementById('btn-toggle-auto-htlt');
const tradeStakeHTLT = document.getElementById('trade-stake-htlt');
const highTickHTLT = document.getElementById('high-tick-htlt');
const lowTickHTLT = document.getElementById('low-tick-htlt');
const maxTradesHTLTInput = document.getElementById('max-trades-htlt');
const loopUntilTargetHTLTCheckbox = document.getElementById('loop-until-target-htlt');
const htltLoopStatus = document.getElementById('htlt-loop-status');
const htltLoopCycleCountDisplay = document.getElementById('htlt-loop-cycle-count');
const htltLoopStatusText = document.getElementById('htlt-loop-status-text');
const HTLT_DURATION_TICKS = 5; // High Tick / Low Tick always run over the next 5 ticks
let htltLoopCycleCount = 0;
let isAutoTradingHTLT = false;
let autoBulkCooldownHTLT = false;
let totalTradesExecutedHTLT = 0;
...
function haltAllAutoModes() {
    ...
    if (isAutoTradingHTLT) toggleAutoHTLT(false);
...
function handleIncomingTickPacket(tickData) {
    ...
    if (!isSessionLocked()) {
        ...
        else if (isAutoTradingHTLT && !autoBulkCooldownHTLT) {
            // Fire the whole High Tick/Low Tick pair batch once on this tick, then auto-stop (or loop).
            executeBulkHighLowTickPair();

            autoBulkCooldownHTLT = true;
            btnBuyHTLT.disabled = true;

            setTimeout(() => {
                autoBulkCooldownHTLT = false;
                if (!isAutoTradingHTLT) btnBuyHTLT.disabled = false;
            }, (HTLT_DURATION_TICKS * 2000) + 1200);

            logToConsole(`[Bulk Auto Triggered] Firing full High Tick/Low Tick pair batch on this tick...`, "system-msg");

            const useLoopModeHTLT = loopUntilTargetHTLTCheckbox && loopUntilTargetHTLTCheckbox.checked;
            if (useLoopModeHTLT) {
                htltLoopCycleCount++;
                if (htltLoopCycleCountDisplay) htltLoopCycleCountDisplay.textContent = htltLoopCycleCount;
                logToConsole(`[Loop Mode] Bulk HTLT cycle ${htltLoopCycleCount} fired. Continuing until session target is hit...`, "system-msg");
            } else {
                toggleAutoHTLT(false);
            }
        }
    }
...
function handlePurchaseReceipt(buyReceipt, passthrough) {
    ...
    const isOver = buyReceipt.shortcode.includes("DIGITOVER") || buyReceipt.shortcode.includes("RUNHIGH") || buyReceipt.shortcode.includes("TICKHIGH");
    const isUnder = buyReceipt.shortcode.includes("DIGITUNDER") || buyReceipt.shortcode.includes("RUNLOW") || buyReceipt.shortcode.includes("TICKLOW");
...
// --- BULK HIGH TICK / LOW TICK ---
// TICKHIGH wins if the selected tick (1-5) is the highest of the next 5 ticks; TICKLOW wins if
// the selected tick is the lowest. One High + one Low is fired per pair, all on the same tick.
function executeBulkHighLowTickPair() {
    if (isSessionLocked()) {
        logToConsole("[Session] Trading is locked until the next session opens.", "error-msg");
        return;
    }
    ...
    const symbol = marketDropdown.value;
    const stake = parseFloat(tradeStakeHTLT.value);
    const clampTick = (el) => Math.min(5, Math.max(1, parseInt(el.value, 10) || 1));
    const highTick = clampTick(highTickHTLT);
    const lowTick = clampTick(lowTickHTLT);
    const currency = currencyText.textContent || "USD";
    const batchSize = parseInt(maxTradesHTLTInput.value, 10) || 1;
    const bulkRunToken = "BULK_HTLT_" + Date.now();
    challengeBatchExpectedCounts[bulkRunToken] = batchSize * 2;
    batchSyncExpected[bulkRunToken] = batchSize * 2;

    // Serialize each side once; the hot loop below is nothing but socket sends.
    const buildPayload = (contractType, selectedTick) => JSON.stringify({
        "buy": 1,
        "price": stake,
        "subscribe": 1,
        "parameters": {
    ...
    const highPayload = buildPayload("TICKHIGH", highTick);
    const lowPayload = buildPayload("TICKLOW", lowTick);
    ...
    totalTradesExecutedHTLT += batchSize * 2;

    logToConsole(`[${bulkRunToken}] Fired ${batchSize} pairs: High Tick (tick ${highTick}) + Low Tick (tick ${lowTick}) (${batchSize * 2} contracts) on this tick. (send loop ${sendMs} ms)`, "success-msg");
}

btnBuyHTLT.addEventListener('click', executeBulkHighLowTickPair);
btnToggleAutoHTLT.addEventListener('click', () => toggleAutoHTLT(!isAutoTradingHTLT));
function toggleAutoHTLT(state) {
    if (state && isSessionLocked()) {
        logToConsole("[Session] Trading is locked until the next session opens.", "error-msg");
        return;
    }
    if (state && !canStartLoopMode(loopUntilTargetHTLTCheckbox, "Bulk HTLT")) return;
    isAutoTradingHTLT = state;
    btnToggleAutoHTLT.textContent = state ? "Stop Auto Bulk Mode" : "Auto Bulk Mode";
    btnToggleAutoHTLT.classList.toggle('stream-active', state);

    tradeStakeHTLT.disabled = state;
    highTickHTLT.disabled = state;
    lowTickHTLT.disabled = state;
    maxTradesHTLTInput.disabled = state;
    if (loopUntilTargetHTLTCheckbox) loopUntilTargetHTLTCheckbox.disabled = state;

    const useLoopModeHTLT = loopUntilTargetHTLTCheckbox && loopUntilTargetHTLTCheckbox.checked;

    if (state) {
        autoBulkCooldownHTLT = false;
        totalTradesExecutedHTLT = 0;
        htltLoopCycleCount = 0;
        if (htltLoopCycleCountDisplay) htltLoopCycleCountDisplay.textContent = '0';
        if (useLoopModeHTLT) {
            if (htltLoopStatus) htltLoopStatus.style.display = 'flex';
            if (htltLoopStatusText) { htltLoopStatusText.textContent = 'Running'; htltLoopStatusText.className = 'success-msg'; }
            logToConsole(`Bulk Auto-Mode Started (Loop Until Target). Will keep firing ${maxTradesHTLTInput.value} High Tick/Low Tick pairs on qualifying ticks until today's session target is hit.`, "success-msg");
        } else {
            if (htltLoopStatus) htltLoopStatus.style.display = 'none';
            logToConsole(`Bulk Auto-Mode Started. Will fire ${maxTradesHTLTInput.value} High Tick/Low Tick pairs on the next qualifying tick, then auto-stop.`, "success-msg");
        }
    } else {
        if (htltLoopStatus) htltLoopStatus.style.display = 'none';
    }
...
function updateTradeControlsState(isActive) {
    ...
    btnBuyHTLT.disabled = !isReady;
    btnToggleAutoHTLT.disabled = !isReady;
    if (!isReady) { toggleAutoEO(false); toggleAutoOU(false); toggleAutoPOU(false); toggleAutoOUD(false); toggleAutoHTLT(false); if (isBulkOver2Armed) disarmBulkOver2(); }
}
...
const SESSION_LOCK_BUTTON_IDS = [
    ...
    'btn-buy-htlt', 'btn-toggle-auto-htlt',
    'btn-toggle-auto-pou',
    'btn-buy-bulk-over2',
