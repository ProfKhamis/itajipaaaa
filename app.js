...
function handleIncomingTickPacket(tickData) {
    ...
    // toString() drops trailing zeros (1234.50 -> "1234.5") which reads the wrong last digit.
    const priceString = (tickData.pip_size !== undefined && tickData.pip_size !== null)
        ? Number(tickData.quote).toFixed(tickData.pip_size)
        : tickData.quote.toString();
    ...
    if (!isSessionLocked()) {
        ...
        if (isAutoTradingEO) {
            ...
            const durationTicksOUD = parseInt(tradeDurationOUD.value, 10);
...
function executeBulkOverUnderPair() {
    ...
    logToConsole(`[${bulkRunToken}] Fired ${batchSize} Over/Under pairs (${batchSize * 2} contracts) on this tick. (send loop ${sendMs} ms)`, "success-msg");
}
...
function executeBulkOnlyUpsDownsPair() {
    ...
    const duration = parseInt(tradeDurationOUD.value, 10);
    const currency = currencyText.textContent || "USD";
